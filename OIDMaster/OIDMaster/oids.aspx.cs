...
namespace OIDMaster
{
    public partial class oids : System.Web.UI.Page
    {
        ...
        protected void Tree1_TreeNodeDataBound(object sender, Obout.Ajax.UI.TreeView.NodeEventArgs e)
        {
            ...
            if (dv != null)
            {
                ...
                e.Node.Text = String.Format("{0} - ({1})", dv["secondary_id"].ToString() + " (" + dv["primary_int"].ToString() + ")", dv["sub_count"].ToString() + " OIDs");
                int parent = 0;
        ...
        {
            Session["find"] = TextBox1.Text.Trim();
