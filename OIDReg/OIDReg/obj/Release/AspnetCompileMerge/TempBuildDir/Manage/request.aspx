                        ...
                        <tr valign="top"  id="tr_item" runat="server">
                            ...
                            <td style="text-align:left">
                                ...
                                <div style="margin:5px; border:1px dashed #eee">
                                    <%#Eval("notation2")%>.<%#Eval("primary_int")%>.<span style="color:blue"><%#Eval("request_oid_int")%></span><br />
                                    <%#Eval("notation1")%> <%#Eval("secondary_id")%> (<%#Eval("primary_int")%>) <span style="color:blue"><%#Eval("request_secondary_id")%> (<%#Eval("request_oid_int")%>)</span>
                                </div>
                            </td>
                            <td>
                                <%#Eval("ustate_desc") %>
                                <div>
                                    <asp:LinkButton ID="linkSubmit" runat="server">ส่งบันทึกการแก้ไข</asp:LinkButton>
                                </div>
                            </td>
                        </tr>
                    </ItemTemplate>
        ...
        <div style="margin:0px 15px;">
            ...
            <div style="margin-bottom:10px;border:0px solid green">
                ...
                <asp:SqlDataSource ID="SqlDataSource2" runat="server" 
                    ConnectionString="<%$ ConnectionStrings:LocalDatabase %>" 
                        SelectCommand="SELECT [node_update].*, primary_int, secondary_id, notation1, notation2, [node_update_status].* FROM node_update INNER JOIN node ON [node_update].[node_id] = [node].[node_id] INNER JOIN [node_update_status] ON [node_update].[status] = [node_update_status].[ustate_id]  WHERE request_by=@raid ORDER BY request_id DESC"   
                    >
                    <SelectParameters>
                        <asp:SessionParameter Name="raid" SessionField="raid" />
                    </SelectParameters>
